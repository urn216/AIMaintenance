package mki.ui.elements;

import java.awt.Font;

import java.awt.Graphics2D;

import mki.ui.components.UIComponent;
import mki.ui.components.UIText;
import mki.ui.components.interactables.UIButton;
import mki.ui.control.UIAction;
import mki.ui.control.UIColours;
import mki.ui.control.UIController;

import mki.math.vector.Vector2;

public class ElemConfirmation extends UIElement {

  protected double buffer;
  protected double componentHeight;

  public ElemConfirmation(Vector2 tL, Vector2 bR, double buffer, byte transition, UIAction yes, UIAction no, String... info) {
    super(tL, bR, transition);
    this.buffer = buffer;
    this.componentHeight = (bR.y-tL.y-(buffer*(info.length+2)))/(info.length*0.5+1);
    this.background = UIBackgrounds.rectangleSolidBack;
    components = new UIComponent[info.length+3];

    for (int i = 0; i < info.length; i++) {
      components[i] = new UIText(info[i], 1, Font.PLAIN);
    }
    components[info.length  ] = new UIButton("Yes"   , yes           );
    components[info.length+1] = new UIButton("No"    , no            );
    components[info.length+2] = new UIButton("Cancel", () -> this.transOut(UIController.getAnimTimeMillis()));
  }

  @Override
  protected void draw(Graphics2D g, int screenSizeY, Vector2 tL, Vector2 bR, UIColours.ColourSet c) {

    float buff = (float) (this.buffer * screenSizeY);

    float x = (float) tL.x + buff;
    float y = (float) tL.y + buff;
    float width = (float) bR.x - buff - x;

    for (int i = 0; i < components.length-3; i++) {
      UIComponent comp = components[i];
      float height = (float) (componentHeight * screenSizeY) / 2;
      comp.draw(g, x, y, width, height, c);
      y += buff + height;
    }

    float height = (float) (componentHeight * screenSizeY);
    width = (float) (bR.x-tL.x-buff*4)/3;

    for (int i = components.length-3; i < components.length; i++) {
      UIComponent comp = components[i];
      comp.draw(g, x, y, width, height, c);
      x += buff + width;
    }
  }
}
