...
public class UIPane {
  ...
  private static UIElement TEMP_ELEMENT_TEMPLATE = new UIElement(new Vector2(), new Vector2(), UIElement.TRANSITION_FADE_IN_PLACE) {
    protected void draw(Graphics2D g, int screenSizeY, Vector2 tL, Vector2 bR, UIColours.ColourSet c) {}
  };

  private UIElement remvElement = TEMP_ELEMENT_TEMPLATE;
  ...
  */
  ...
  public void reset() {
    clear();
    setState(UIState.DEFAULT, UIController.getAnimTimeMillisLong());
  }
  ...
  public void setTempElement(UIElement tempElement) {
    this.remvElement = this.tempElement;
    this.remvElement.transOut(UIController.getAnimTimeMillis());
  ...
  public void clearTempElement() {
    setTempElement(TEMP_ELEMENT_TEMPLATE);
  }
  ...
  public void transOut() {
    for (UIElement e : elements) {
      e.transOut(UIController.getAnimTimeMillis());
    }
  }
  ...
  public void transIn() {
    for (UIElement e : elements) {
      e.transIn(UIController.getAnimTimeMillis());
    }
  }
  ...
  public void retState() {
    setState(states.get(currentState).getParent(), UIController.getAnimTimeMillis());
  }
  ...
  public void setState(UIState state, long animTimeMillis) {
    if (!isTransitioning()) forceState(state, animTimeMillis);
  }
   ...
   * @param animTimeMillis the length of the transition in milliseconds
   */
  public void forceState(UIState state, long animTimeMillis) {
    ...
    tempElement.transOut(UIController.getAnimTimeMillis());
  }
  ...
  /**
   * Sets an existing state's back action.
   * 
   * @param state the existing state to change the back action of
   * @param back the desired action to perform on backing out of the mode
   */
  public void setModeBackAction(UIState state, UIAction back) {
    Mode mode = states.get(state);
    if (mode == null) return;
    mode.setBack(back);
  }
  ...
  /**
  * Looks through all the elements in this pane to retrieve the top-most {@code UIComponent} at a given location
  *
  * @param x the x coord to check
  * @param y the y coord to check
  *
  * @return the top-most {@code UIComponent} present at this location, or null if none applicable
  */
  public UIComponent getComponent(double x, double y) {
    if (tempElement.isActive()) {
      UIComponent c = tempElement.getComponent(x, y);
      if (c != null) return c;
    }

    for (int i = elements.size()-1; i >= 0 ; i--) {
      if (!elements.get(i).isActive()) continue;
      UIComponent c = elements.get(i).getComponent(x, y);
      if (c != null) return c;
    }
  ...
  */
  ...
  public void draw(Graphics2D g, int screenSizeX, int screenSizeY) {
    for (UIElement e : elements) {
      e.draw(g, screenSizeX, screenSizeY);
    }
    
    remvElement.draw(g, screenSizeX, screenSizeY);
    tempElement.draw(g, screenSizeX, screenSizeY);
  }
}
...
class Mode {
  ...
  public void setBack(UIAction back) {
    this.back = back;
  }
}
